...
% freq = 2*pi*freq;
Vout = [1.05 1.17 0.752 0.282 0.138 0.0218; 1.06 1.19 1.33 0.405 0.193 0.0266; 1.05 1.17 2.54 0.556 0.246 0.031];
...
Tf1 = feedback(k(1)*tf([3.712e6],[1 2568 3.705e6])*tf(1,[7.3e-4 0]), 1)

X2 = fminsearch('A_bode_f_B', Xo2);
Ls2 = [k(2)];
%Ms2 = [X2(2) X2(3) X2(4) X2(5)];
%Tf2 = feedback(tf([Ls2],[Ms]), 1);
Tf2 = feedback(k(2)*tf([3.712e6],[1 2568 3.705e6])*tf(1,[7.3e-4 0]), 1)

X3 = fminsearch('A_bode_f_C', Xo3);
Ls3 = [k(3)];
%Ms3 = [X3(2) X3(3) X3(4) X3(5)];
%Tf3 = feedback(tf([Ls3],[Ms]), 1);
Tf3 = feedback(k(3)*tf([3.712e6],[1 2568 3.705e6])*tf(1,[7.3e-4 0]), 1)
